...
void cor_tests(double (*cor)(double, double, int,int)){
	...
	SECTION("Corrections must be symmetric."){
		for(int i=0;i<100;i++){
			...
			REQUIRE(approx_equal(cor(elo1,elo2,r1,r2),-cor(elo2,elo1,r2,r1)));
		}
	}
...
TEST_CASE("Expected outcome behaves sensibly","[expected_outcome]"){
	...
	SECTION("Equal Elo scores results in draw"){
		REQUIRE(approx_equal(expected_outcome(1321,1321),0.5));
	}
}
